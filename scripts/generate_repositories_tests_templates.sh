...
# Find all .java files in the 'repository' subdirectories in 'main'
find "$MAIN_DIR" -type f -path "*/repository/*.java" | while read repository_class; do
  # Extract the relative path from the base directory (i.e., relative to src/main/java/)
  relative_path="${repository_class#$MAIN_DIR/}"
  
  # Get the class name (basename without extension)
  class_name=$(basename "$repository_class" .java)
