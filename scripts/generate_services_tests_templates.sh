...
# Find all .java files in the 'service' subdirectories in 'main'
find "$MAIN_DIR" -type f -path "*/service/*.java" | while read service_class; do
  # Extract the relative path from the base directory (i.e., relative to src/main/java/)
  relative_path="${service_class#$MAIN_DIR/}"
  
  # Get the class name (basename without extension)
  class_name=$(basename "$service_class" .java)
