...
# Find all .java files in the 'controller' subdirectories in 'main'
find "$MAIN_DIR" -type f -path "*/controller/*.java" | while read controller_class; do
  # Extract the relative path from the base directory (i.e., relative to src/main/java/)
  relative_path="${controller_class#$MAIN_DIR/}"
  
  # Get the class name (basename without extension)
  class_name=$(basename "$controller_class" .java)
